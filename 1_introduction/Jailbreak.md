...
-->
> This document is based on PS4 firmware 6.72 version. **The goal of this project is finding vulnerabilities, checking posibility of jailbreak on latest firmware and protecting intellectual properties formed on PS4 platform, not abusing jailbreak.** The document describes about some materials and tools referenced to achieve the above goal. **We declare that we will not take any responsibility for your disadvantage of abusing the tools described in this document.**

## 2. What can we do using Jailbreak
<!--
...
-->
First of all, we conducted 6.72 version jailbreak to see what functions were available through jailbreak. We followed this [video](https://www.youtube.com/watch?v=ycZg0fViWv4). There are some functions on the jailbreak site as below, I would like to introduce some of them that can help your analyze.

![jailbreak](https://user-images.githubusercontent.com/40509850/101586509-fec5e280-39d9-11eb-9012-09aa04601f6d.PNG "Figure 1 The function of jailbreak tool")

### 2.1. Execution arbitrary binary
<!--
...
-->
Bin Loader executes arbitrary binary payload. Execute Bin Loader and then send a binary file using command `nc -w 3 (PS4's ip) 9021 < (bin payload)` to execute payload. Arbitrary binary payload can be made using [8cc](https://github.com/sleirsgoevy/ps4-rop-8cc) that is part of the tool sleirsgoevy made. The maximum size of binary payload is 65536 byte.<sup id="head1">[1](#foot1)</sup>

### 2.2. Browsing the files in PS4
<!--
...
-->
As the name implies, FTP activates FTP and opens 1337 port. We can browse the file system in the PS4. You can easily access it by using the program like [FileZilla](https://filezilla-project.org/). However, it seems that we can not use the library and binary files directly because the they are encrypted.

### 2.3. UART Log Activation
<!--
...
-->
Mira not only unlocks 'Debug Settings' option that has several developer tools but also activates UART Log. You can see the UART Log by connecting UART pin on the board. However, UART Log port is also opened so you don't need to connect UART pin physically. You can see the log using command `nc (PS4's IP) 9998`. The IP of PS4 is in `Settings > System > System Information`.<br>
The log gives you information such as hardware information, game title code, and so on. The interesting part is that **if a process crashes, the log gives you information such as register, backtrace, and so on like below.** This makes analysis easier.
